...
export async function notifyDiscord(
  ...
  if (!webhookUrl) {
    logger.warn(
      ...
      "No Discord webhook configured for severity"
    );
    return null;
  }
  ...
  const embed: DiscordEmbed = {
    title: `${
      SEVERITY_EMOJIS[errorLog.severity]
    } ${errorLog.severity.toUpperCase()}: ${errorLog.error_type}`,
    ...
    fields: [
      ...
      {
        name: "Environment",
        value: errorLog.environment || "unknown",
        inline: true,
      },
      {
        name: "Occurrences",
  ...
  if (errorLog.url) {
    embed.fields.push({
      name: "URL",
      value: truncate(errorLog.url, 100),
      inline: false,
    });
  }
